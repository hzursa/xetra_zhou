...
class XetraETL():
    ...
    def __init__(self, s3_bucket_src: S3BucketConnector,
        ...
        self.extract_date,self.extract_date_list = MetaProcess.return_date_list(
            src_args.src_first_extract_date,self.meta_key,self.s3_bucket_trg)
    ...
    def extract(self):
        ...
        if not files:
            ...
            data_frame=pd.concat([self.s3_bucket_src.read_csv_to_df(key) for key in files],ignore_index=True)
        self._logger.info('Extracting Xetra source files done.')
        return data_frame
    ...
    def transform_report1(self,data_frame:pd.DataFrame):
        ...
        data_frame=data_frame\
            .groupby([
                ...
                .agg({self.trg_args.trg_col_op_price:'min',
        ...
        data_frame[self.trg_args.trg_col_ch_prev_clos]= data_frame\
            .sort_values(self.src_args.src_col_date)\
    ...
    def load(self,data_frame: pd.DataFrame): 
        ...
        # Updating meta file
        MetaProcess.update_meta_file(self.meta_key,self.meta_update_list,  self.s3_bucket_trg)
