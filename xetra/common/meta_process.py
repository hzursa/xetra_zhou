...
class MetaProcess():
    ...
    use static method in order to use them as usual function
    """

    @staticmethod
    def update_meta_file(meta_key: str,report_date_list,s3_bucket_meta: S3BucketConnector):
        """
        ...
        """
        df_new = pd.DataFrame(columns=[MetaProcessFormat.META_SOURCE_DATE_COL.value,MetaProcessFormat.META_PROCESS_COL.value])
        df_new[MetaProcessFormat.META_SOURCE_DATE_COL.value] = report_date_list
    ...
    @staticmethod
    def return_date_list(first_date,meta_key: str,s3_bucket_meta: S3BucketConnector):
        """
        Create a list of date strings based on the input first_date and the already
        processed data in the meta file
        ...
        # when it run for the first time, there are no meta file
        try:
            ...
            if dates_missing:
                ...
                return_dates = [
                    dates.strftime(MetaProcessFormat.META_DATE_FORMAT.value) \
                    for dates in dates if dates >= min_date
                    ]
                #this arg is used to filter the final report
